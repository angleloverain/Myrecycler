...
public class MySwipeRefreshLayout4 extends ViewGroup implements NestedScrollingParent {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        headTop -= changeSize;
        headBottom -= changeSize;  //  这个就是recycle的 Top
        footTop -= changeSize;     //  这个就是recyvleView bottom
        footBottom -= changeSize;

        if ((isLongUpLayout &&(headTop<normalTop || footBottom < normalbottom)) ||
            (isLongDownLyaout &&(headTop>normalTop || footBottom > normalbottom)))
        {
            headTop = normalTop;
            headBottom = 0;
